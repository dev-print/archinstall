...
def define_arguments() -> None:
	...
	parser.add_argument("--skip-version-check", action="store_true",
						help="Skip the version check when running Catinstall")
...
if 'sphinx' not in sys.modules and 'pylint' not in sys.modules:
	if '--help' in sys.argv or '-h' in sys.argv:
		...
		exit(0)
	if os.getuid() != 0:
		print(_("Catinstall requires root privileges to run. See --help for more."))
		exit(1)
...
def load_config() -> None:
	...
	if (archinstall_lang := arguments.get('Catinstall-language', None)) is not None:
		arguments['Catinstall-language'] = translation_handler.get_language_by_name(archinstall_lang)
...
def run_as_a_module() -> None:
	...
	try:
		...
		if exc:
			...
			text = (
				'Catinstall experienced the above error. If you think this is a bug, please report it to\n'
				'https://github.com/dev-print/archinstall and include the log file "/var/log/archinstall/install.log".\n\n'
